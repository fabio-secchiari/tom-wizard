...
être assuré que toutes les fonctionnalités et l’interface utilisateur fournies ont été rédigées par un être humain.
...
\textit{Si vous souhaitez suggérer une nouvelle fonctionnalité, signaler un bug ou même une faute de frappe dans ce
=======
...
\textit{Le présent manuel d’utilisation est mis à jour pour la version 2.4 du micrologiciel ESP à la date du 03/09/2026.}
=======
...
<<<<<<< HEAD
Comme le tableau de bord de notre Twizy n’affiche que quelques valeurs, telles que la vitesse et les
...
en 3D.
Comme sur le tableau de bord d’origine, vous y trouverez l’état de la batterie, le rapport engagé, l’autonomie restante, la vitesse et l’heure.
...
<<<<<<< HEAD
exemple, cet appareil peut être utilisé pour transférer des données depuis votre Twizy vers votre \textbf{MQTT}
...
dans votre navigateur préféré l’adresse IPv4 privée statique de ToM (\textbf{\textit{192.188.1.188}}) et de profiter de la navigation !
=======
...
<<<<<<< HEAD
Comme vous pouvez le constater dans cette nouvelle version de ToM, le serveur Web est bien plus complet et convivial. Vous pouvez facilement naviguer parmi les pages et configurer votre appareil comme vous le souhaitez. Le serveur Web est également adapté aux appareils mobiles, vous pouvez donc l’utiliser sur votre smartphone ou votre tablette.

\subsection{Application mobile ToM}

Parallèlement aux mises à jour du firmware de l’écran LCD et de la « boîte noire », une application Android minimaliste est
...
<<<<<<< HEAD
sur l’application et sur la page MQTT de ToM+, en intervertissant les sujets IN et OUT. L’application est
=======
...
<<<<<<< HEAD
portant l’extension \textit{(.apk)}. Au premier démarrage, vous verrez peut-être s’afficher une fenêtre contextuelle indiquant que cette application
a été conçue pour une version plus ancienne d’Android si vous possédez un téléphone plus récent : cliquez simplement sur OK
=======
...
<<<<<<< HEAD
    \caption{Consommation lorsque le ToM+ est allumé avec le Wi-Fi}
=======
...
<<<<<<< HEAD
et les capteurs des chocs pendant le transport. Dans cette nouvelle version, plus besoin de longs câbles : un seul câble plus court, relié à la carte mère interne du ToM+, est logé dans un petit boîtier en plastique noir
que j’appelle généralement la « boîte noire ». Bien sûr, l’écran tactile LCD résistif est également fourni ; il vous permet de visualiser et de configurer l’ensemble du système.
=======
...
<<<<<<< HEAD
La « boîte noire » est la carte mère de ToM+ et contient la plupart des composants nécessaires
=======
...
la broche 1 (anciennement jaune) et la broche 2 (anciennement bleue). Elles peuvent remplir des fonctions d’entrée ou de sortie, selon votre choix sur la \textit{« page d’extension »} du serveur web ToM \textbf{.} Dans cette dernière version de l’appareil, le connecteur d’extension est intégré au boîtier noir (sans câbles colorés), mais le brochage reste identique.
N’oubliez pas de retirer le capot de protection avant de l’utiliser et de respecter une tension d’entrée de \textbf{3,3 V maximum}. Pour des tensions plus élevées,
...
<<<<<<< HEAD
Dans la version autonome, vous pouvez le fixer à l’aide du cache et des vis fournis dans l’emballage, ainsi que du support pliable en 3D, comme le montrent les images ci-dessous.
=======
...
<<<<<<< HEAD
Le kit comprend un serre-câble permettant de fixer le support imprimé en 3D pour le bouton sur la manette droite d’essuie-glace (également fournie). De plus, un autocollant vous est offert si vous souhaitez soutenir le projet en le collant quelque part sur votre Twizy.
=======
