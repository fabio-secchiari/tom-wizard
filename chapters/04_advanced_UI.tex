...
\begin{itemize}
    ...
    \item \textit{icon x fix}: icon x (\textit{where x can be 2,3,4 or 5}) lights on. See Figure~\ref{fig:alarm_icons} for alarm icons.
    ...
    \item \textit{pin2 active}: PIN2 of the expansion board gets activated. In this example PIN2 was set as INPUT mode so it's greyed out, because it can't represent an OUTPUT.
\end{itemize}
\  \\
\begin{figure}[ht]
    \centering
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Alarm icons (2,3,4 and 5).}
    \end{subfigure}
\end{figure}