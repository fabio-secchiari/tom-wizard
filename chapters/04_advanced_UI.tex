...
While the second column under the \textbf{“KEY”} header is for the Wi-Fi password.\\ \todo{TODO}
...
The first flag, \textit{Disable Auto-dimming}, can be checked if you don't like ToM+
LCD display to adapt the brightness when turning on the light beams.
Otherwise, you can adjust the \textit{Dimming brightness (\%)} expressed in percentage (up to 100\%),
...
select the reminders (light beam and handbrake) and info (system errors and charge limits)
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{figures/pop-up_finale.png}
    \caption{Warning message at power-off.}
\end{figure}

Another available flag is \textit{Invert current value sign}. If checked, it basically shows
...
You can set your \textit{Cheating factor} multiplied by 10: for example, 2.0 becomes 20.
You can also select if the Cheatzy is enabled when charging or during regenerative braking
using the \textit{Cheat also Charge/Reku} flag. These settings only make sense if the Cheatzy 
...
occurs (default: one per minute) in the \textit{Remaining KM sampling time (min)} and
how much the last sample affects the total calculation (default: 10\%) in the
\textit{Remaining KM sample weight (\%)}.\\

Lastly we have the \textit{Miles to Kilometers} flag, that helps when your BigToM
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/charging_history.png}
...
flag, the \textbf{“Enable auxillary display”} one.\\
