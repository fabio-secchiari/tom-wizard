...
als Englisch wurden automatisch mit} \href{https://www.deepl.com/en/translator}{DeepL} übersetzt.\\

\textit{In einer Zeit der automatisierten Inhaltserstellung hebt sich dieses Handbuch deutlich ab, und Sie können
sicher sein, dass alle beschriebenen Funktionen und die Benutzeroberfläche von Menschen verfasst wurden.
Dies ist es, was dieses Projekt unter allen Alternativen einzigartig macht: Genau zu wissen,
was der Code tut, ist der Schlüssel dazu, bewusst neue Funktionen zu entwickeln, Fehler zu beheben und
bei Bedarf anderen ToM+-Besitzern Unterstützung zu gewährleisten.}\\

\textit{ToM+ entwickelt sich dank seiner großen Community, die aus dem}
\href{https://www.twizy-forum.de/twiz-o-meter}{deutschen Twizy-Forum} hervorgegangen ist, ständig weiter. \textit{Wenn Sie 
...
Handbuch melden möchten, senden Sie mir bitte eine E-Mail oder eröffnen Sie einen Pull-Request auf} \href{https://codeberg.org/fabio-java/ToM-Wizard}{Codeberg}.\\
...
Exemplar ist ein Unikat und muss als Prototyp betrachtet werden, ohne CE-Kennzeichnung oder sonstige 
...
und Nutzung. ICH FÜGE EINEN TESTSATZ HINZU! DIE KATZE SITZT AUF DEM TISCH UND DER HUND LIEGT IM BETT}\\
...
im linken Handschuhfach ausliest. ToM „abfängt“ einfach die vom Twizy gesendeten Werte und
...
Das allgemeine Erscheinungsbild der anderen Seiten ist in der Abbildung dargestellt, wobei ein Wert im Vordergrund und die anderen auf der linken Seite angezeigt werden. Unten
...
Das in der schwarzen Box enthaltene ESP32-Board verfügt über die Möglichkeit, eine Verbindung zu einem
WLAN-Zugangspunkt herzustellen. Sobald das ToM+ verbunden ist, stehen Ihnen zahlreiche Möglichkeiten offen. So
kann dieses Gerät beispielsweise dazu verwendet werden, Daten von Ihrem Twizy an Ihren persönlichen \textbf{MQTT}-Broker
zu übertragen, wo Sie diese Werte nutzen können, um Ihr Fahrzeug in Ihr IoT-System für die Hausautomation einzubinden. 

Dies ist nicht die einzige Funktion im Zusammenhang mit der WLAN-Fähigkeit, über die ToM+ verfügt. Es kann 
auch als Zugangspunkt mit eigener SSID und eigenem Passwort fungieren und so ein privates Netzwerk ohne 
Internetverbindung schaffen. Auf diese Weise können Sie eine sichere Verbindung herstellen und \textbf{akzeptieren, das Verbindungsprofil}
 aktiv zu halten, selbst wenn keine Internetverbindung besteht (dies wird in der Regel durch eine
Benachrichtigung auf Android-Smartphones 10 Sekunden nach der ersten Verbindung) und anschließend Ihre
weiteren Konfigurationen auf einigen Webserver-Seiten vornehmen, die von ToM+ selbst gehostet werden. Geben Sie einfach
...
\subsection{ToM-Smartphone-Anwendung}

Neben den Firmware-Updates für das LCD-Display und die Blackbox wird auch eine minimalistische Android-Anwendung
...
als \textbf{APK}-Datei bereitgestellt, einem gepackten Installationsprogramm für Android-Anwendungen, das
es Ihnen ermöglicht, diese App auch ohne Internetverbindung zu installieren. Sie ist \href{https://www.mediafire.com/file/du7px1bzq0zzdxu/TwizOMeter_v3.1.2.apk/file}{hier klickbar} verfügbar.
...
mit der Erweiterung \textit{(.apk)} zu öffnen. Beim ersten Start wird möglicherweise ein Popup-Fenster angezeigt, in dem darauf hingewiesen wird, dass diese Anwendung
für eine ältere Android-Version erstellt wurde, falls Sie ein neueres Smartphone besitzen: Klicken Sie einfach auf „OK“
und ignorieren Sie die Meldung (sie funktioniert einwandfrei). Hier einige Vorschau-Bilder der Lade- und Einstellungsseite der App:
...
ToM nutzt die 12-V-Batterieversorgung, die direkt über den OBD-Stecker bezogen wird, und
verbraucht nur wenig Strom, wie die folgenden Messwerte zeigen.
...
Finger oder einen kleinen Stylus-Stift aus hartem Kunststoff zum Betätigen der Tasten verwenden. Es ist zwar nicht so leistungsstark wie
...
und Sensoren vor Stößen während des Transports zu schützen. In dieser neueren Version sind keine langen Kabel mehr erforderlich, sondern nur noch ein kürzeres, das an die interne ToM+-Hauptplatine angeschlossen ist und in einem kleinen schwarzen Kunststoffgehäuse
untergebracht ist, das ich gewöhnlich als „Black Box“ bezeichne. Selbstverständlich ist auch das resistive LCD-Touchdisplay im Lieferumfang enthalten, auf dem Sie alle Einstellungen anzeigen und konfigurieren können.
Sie benötigen keine Schrauben oder Klebeband mehr! Das Display lässt sich einfach aufstecken und passt direkt auf das ursprüngliche Armaturenbrett.
...
\begin{itemize}
    ...
    \item Einen Erweiterungsstecker mit Stromversorgungs- und GPIO-Pins;
...
Wenn Sie Ihr ToM+ weiter anpassen möchten, können Sie dies ganz einfach mithilfe des
hier abgebildeten Steckers tun. Tatsächlich stellt er eine zusätzliche 5-V-Stromversorgung bereit, die von
VCC (z. B. rotes Kabel) und GND (z. B. schwarzes Kabel) stammt, sodass Sie eine externe LED, einen
kleinen Summer oder ein beliebiges anderes Gerät Ihrer Wahl mit Strom versorgen können. Die beiden anderen Kabel sind allgemeine Ein- und Ausgangspins:
PIN1 (z. B. gelb) und PIN2 (z. B. blau). Sie können entweder als Ein- oder als Ausgang fungieren, je nachdem, welche Einstellung Sie auf der \textit{„Erweiterungsseite“} des ToM-\textbf{Webservers} wählen. In dieser neuesten Version des Geräts ist der Erweiterungsstecker in das schwarze Gehäuse (ohne farbige Kabel) integriert, die Pinbelegung bleibt jedoch unverändert.
...
Das Display, das – wie auf den Bildern zu sehen – mit einer Schutzfolie versehen ist, lässt sich in den 3D-Halter einsetzen, um einen verstellbaren Ständer zu erhalten, den Sie an einer beliebigen Stelle aufstellen können.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/3d_stand_wdisplay.png}
    \caption{LCD-Display als Standalone-Version}
\end{figure}

In der Standalone-Version können Sie das Display mit der Abdeckung und den Schrauben befestigen, die Sie zusammen mit dem zusammenklappbaren 3D-Ständer in der Verpackung vorfinden, wie auf den folgenden Abbildungen dargestellt.
...
Der 3D-Ständer verfügt außerdem über Bohrungen an der Basis, falls Sie ihn am Handschuhfach befestigen möchten. Die übrigen Bohrungen am Rahmen dienen dazu, die Rückabdeckung festzuschrauben, um das Display im Inneren an seinem Platz zu halten.
...
Im Lieferumfang ist ein Kabelbinder enthalten, mit dem der 3D-gedruckte Ständer für den Knopf am rechten Scheibenwischerhebel (ebenfalls im Lieferumfang enthalten) befestigt werden kann. Darüber hinaus liegt ein Bonus-Aufkleber bei, falls Sie das Projekt unterstützen möchten, indem Sie ihn an einer beliebigen Stelle Ihres Twizy anbringen.
