...
The fourth WiFi connection profile is special, because the \textbf{“KEY 4”} field is
used both as a standard password field for the last SSID and to change the default webserver password,
as will be discussed in Section~\ref{sec:web_change_password}.

In addition, there are some special codes you can enter in this field to perform some
secret configuration. These codes are case sensitive and won't affect your \textbf{“KEY 4”}
field if written correctly, since ToM+ uses them as commands only. You can type
\textbf{GIVE\textunderscore ME\textunderscore DF!} (exactly as wrote here, with underscore
and exclamation mark too) to \textit{show DF error numbering and error decription} both in 
display and webUI.\\

\textbf{REMEMBER!} The commands are executed by ToM+ only if they are entered from
this WiFi settings page, not from the webUI discussed later. In addition, since the
origin of DF number is not clear, enabling this function is up to you and under 
your responsibility.
...
We will see more interesting features in which ToM+ IP address is involved later at Section~\ref{sec:web_server_access}.
...
This is useful if you want to publish real-time data to your smartphone via MQTT.~This
also enables the web interface to be accessed while driving and your ToM+ will
...
\begin{figure}[H]
    ...
    \caption{Step 3: Add IN/OUT topics.}\label{sec:free_broker_add_topics}
\end{figure}
...
{BigToM / ToM+ Sets}{Here you can configure some dashboard parameters, such as the scale of the power bar or setting up a speed limit (Section~\ref{sec:web_btom_tom_sets}).}
...
\begin{itemize}
    ...
    \item \textbf{KEY+ CONTROLLER}: This mode allows you to control your ToM+ via a rotational wireless remote. For these two first modes an additional configuration table will appear.
...
another type of controller (the one in the picture below). It's a wireless 
rotary controller for car audio systems, which performs different
...
This works the same as the IR settings table. The main difference is about what ToM+ reads
from the expansion port, that is not an IR code anymore: it's a voltage instead.

In the image above are shown the default values, ranging from \textit{0} to \textit{3.3V}
written without the decimal dot symbol. For example, \textit{018} means \textit{0.18V}
and \textit{157} is \textit{1.57V} and so on.
ToM+ has a tolerance of \textit{0.05V} on input values to avoid voltage bouncing.\\

After that, remeber to press the \textbf{“SAVE”} button to save your changes, otherwise they will be lost when
you leave the page. Lastly, a useful \textbf{“Default”} button is located next to it, to easily revert your
changes if needed.
...
When something happens on ToM+ and you want to get noticed about it, you can
choose how to be notified and when in this page. You can have up to \textbf{ten
alarms} running simultaneously and TOM+ constantly checks the state of each condition.
...
\begin{figure}[H]
    ...
    \caption{Alarm page --- Existing alarms list table.}
\end{figure}

The headers of the tables are practically the elements of the condition that ToM+ listens for.\\

\begin{itemize}
    \item \textbf{ITEM}: This is the data to be checked, chosen from the list of Section~\ref{sec:icon_list}.
    \item \textbf{CONDITION}: This is the operator sign used when comparing the values.
    \item \textbf{VALUE}: This is a signed integer value used as the second term of the comparison.
    \item \textbf{ACTION}: This is the action performed when the alarm triggers. See the next table.
    \item \textbf{SEND-MQTT}: This is a binary field and if crossed, pulish a message on MQTT topic.
    \item \textbf{COMMAND}: This is an optional string to be sent as an additional command.
\end{itemize}
\  \\
The first alarm trigger checks the battery consumption and activates the PIN2 of the 
expansion board if it's greater than 35A. In this specific example, it is used to power
on the rear brake light when the regeneration is enough to make the Twizy slow down.\\

The second alarm trigger checks the battery SOC and shows on the display a simple pop-up
that notify that the battery level is lower than 30\%. By checking the \textit{“Change page”} field,
I added a further option that will tell ToM+ to send a MQTT message when this alarm triggers.\\

The third alarm trigger checks the battery SOC and once charge is complete,
e.g.~equal 97\%, it does the same as before but performs an additional command. The
command \textit{cmd/charge/Power 0} disables a Tasmota Wi-Fi switch that connects the
Twizy plug to the wall socket.

So, the function of the \textbf{“COMMAND”} column is really useful and allows you to embed 
your ToM+ in your IoT house. See the Appendix~\ref{apx:tasmota} for detailed configurations.\\

When you want to remove a specific alarm you will just need to press the \textbf{“Remove”} button in its last column.\\


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/alarm_page2.png}
    \caption{Alarm page --- Adding new alarm table.}
\end{figure}

In the second table you can add a new alarm to the list we have just discussed.
First of all you have to choose the \textbf{ITEM}, between all the available data listed in Section~\ref{sec:icon_list}.
In this choice box items are organized as in the list just quoted.
Then select the \textbf{CONDITION}, i.e.~the operator of the comparison, between \textit{“lower”}, \textit{“greater”} or \textit{“equal”}.
Next it's time for the reference \textbf{VALUE}, a signed integer number, the right term of the comparison.\\

Let's take some time to explain all of the possible \textbf{ACTION}s.\\

\begin{itemize}
    \item \textit{pop-up}: a yellow pop-up notifying the item value appears in the center of the screen. To hide it just press it once. You can see a pop-up example in the first image below.
    \item \textit{pop-up 10 sec}: a yellow pop-up notifying the item value stays on the screen for ten seconds.
    \item \textit{silent}: no visible actions happen: it's commonly used to post MQTT messages only.
    \item \textit{icon x fix}: icon x (\textit{where x can be 2,3,4 or 5}) lights on. See Figure~\ref{fig:alarm_icons2} for alarm icons.
    \item \textit{icon x blink}: icon x (\textit{where x can be 2,3,4 or 5}) starts blinking.
    \item \textit{pin1 active}: PIN1 of the expansion board gets activated (LOW or HIGH depending on the level selected in the expansion settings page in Section~\ref{sec:web_alarm_triggers}).
    \item \textit{pin2 active}: PIN2 of the expansion board gets activated. In this example PIN2 was set as INPUT mode so it's greyed out, because it can't represent an OUTPUT action.
\end{itemize}
\  \\
\begin{figure}[ht]
    \centering
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Pop-up: Battery temp > 89°C.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/alarm_icons.png}
        \caption{Alarm icons (2,3,4 and 5).}\label{fig:alarm_icons2}
    \end{subfigure}
\end{figure}

The next field is the binary \textbf{SEND MQTT} flag: if enabled, ToM+ published a MQTT
message on the configured MQTT out topic (e.g. TOMout, see Section~\ref{sec:mqtt_connection})
when the alarm triggers as an additional notify. Last but not least is the \textbf{AUX CMD}
text field, that holds an additional command to be sent via MQTT to other IoT devices.
See the Appendix~\ref{apx:tasmota} for detailed configurations of a Tasmota switch as an example.\\

Finally you can add your alarm by pressing the \textbf{“Add to list”} button. The page
automatically reloads and the alarm trigger is appended to the list in the first table.

Press the \textbf{“HOME”} green button at the end of the page to return to the landing page.


\subsection{Alarms log page}
\label{sec:web_alarms_log}

This page stores the last 21 alarm triggers and does the same function as the log
page of ToM, as shown in Section~\ref{sec:alarm_log}. You can see and remove any 
unwanted records as well, by pressing the \textbf{“Remove”} button near the one you
would like to permanently delete (on ToM+ display too).\\

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/alarm_log1.png}
    \caption{Alarm log page table.}
\end{figure}

Obviously the table holds the \textbf{ITEM} and \textbf{VALUE} that triggered the alarm.
But there are additional fields with the date and time when the trigger happened.
Then, as usual, press the \textbf{“HOME”} green button at the end of the page to return to the landing page.


\subsection{OTA Update \& Preferences page}
\label{sec:web_ota_update}

In this page you can \textbf{update the firmware} on the fly from the web interface, without having to
struggle with the flash tool as in the older versions. This new feature is time saving
and much more user friendly, so be sure to have always the latest version of the firmware
installed, to have access to the newest features and improvements!

\subsubsection{OTA Update}

The first table gives you some information about the blackbox firmware: let's discuss
these values. The \textbf{ESP fw} is probably the most important item in the grid because
it tells you the version that is currently installed and loaded on the ESP motherboard.

Another useful field is the \textbf{S/N} that stands for Serial Number, that you can find
on ToM+ as well as we seen in Figure~\ref{fig:settings_page}. The next row is a little
more technical and it is strictly related to the OTA (Over-The-Air) update mechanism.
\textbf{Current ptn} and \textbf{Next ptn} refer to the partitions addresses where the
firmware is currently stored and where it will be stored while the update is in progress.\\

Sometimes firmware patches, like bug fixes or minor stuff, may be released without
changing the version number (e.g. 2.4). In these cases, it can be useful knowing which
specific binary file is installed, so the next two fields hold its complete name.
As you can see, there are two main firmware partitions, \textbf{Ota0} and \textbf{Ota1},
so that you can have two different versions installed and change between them if needed.
Here, the full name is \textit{ESP32\_Fw24\_70\_4error12} for both.

The last field is the complete name of the \textbf{Special partition} firmware, which
is an additional storage used to hold third-party softwares (e.g.~the Twizy-cfg tuning software).
For more information about tuning your Twizy using Twizy-cfg on ToM+ refer to Appendix~\ref{apx:tuning}.
...
The second part of this table allows you to update the firmware or load a third-party
software. First of all download the binary file (with the .bin extension) you want to 
install on ToM+. Then press the \textbf{“Browse\ldots”} button and navigate your device
folders until you find the desired file.

If it is a \textit{standard ToM+ update}, go and press the \textbf{“UPDATE”} button. To check
the update status, an orange progress bar will appear at the bottom of the table with
its percentage. When it reaches 100\% ToM+ will eventually reboot to apply the changes.

If it is a \textit{third-party software}, be sure to check the \textit{“Update to special partition”}
option and, only after that, press the \textbf{“UPDATE”} button. As soon as the update
is completed ToM+ will reboot and load the special partition firmware.\\

In the \textbf{“BOOT OPTIONS”} table instead, once you have loaded the desired firmwares, you can switch
bewteen them if needed. Let's say I'm running ToM+ 2.4 firmware and I want to do some
tuning with Twizy-cfg third-party software; I have already loaded both of them in the correct
partitions. In order to achieve this, press the \textit{“Reboot from special partition”}
button and wait for ToM+ to reboot, then follow the Appendix~\ref{apx:tuning} for more.\\

\textit{“Reboot from Ota0 partition”} and \textit{“Reboot from Ota1 partition”} buttons
are used to change between ToM+ firmware version. Let's say I'm running ToM+ 2.4 firmware
but for some reason I want to get back to the 2.3 version I had before. To do so, press
the respective button to reboot from either Ota1 and Ota0. ToM+ will instantly reboot
and you will get disconnected to the web interface.
...
Moving to the next one, we have the \textbf{“LCD FIRMWARE UPDATE”} table, that allows
to perform OTA updates for the LCD display as well. This means no more struggling with
the microSD card and the little hole on the display.\\

\textbf{WARNING:} As written in the table you \textbf{MUST NOT} close this web page
while updating: disabling your screen saver is adviceable to avoid update failure.
The process may take some minutes, depending on the firmware version, up to 10 minutes.\\

This is similar to the blackbox OTA update. So, first of all download the file (with the .tft extension) you want to 
install on ToM+. Then press the \textbf{“Browse\ldots”} button and navigate your device
folders until you find the desired file.

If you are \textit{updating the main display}, go and press the \textbf{“UPDATE LCD”} button.
To check the update status, the blue progress bar under the disclaimer box will show
chunks copied to the LCD.~When it finishes the display will reboot (not the blackbox).

If you are \textit{updating the twin display}, be sure to check the \textit{“Update AUX LCD”}
option and, only after that, press the \textbf{“UPDATE LCD”} button. As soon as the update
is completed the LCD display will reboot and load the desired firmware.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/ota_lcd.png}
    \caption{OTA Update page --- LCD firmware update table.}
\end{figure}


\subsubsection{Preferences and HW Upgrade}

In the next web page section we can manage preferences, which includes WiFi and MQTT
configurations, ToM+ pages icon arrangements and even more. So, it is always adviceable
to have a \textbf{backup before updating}, to avoid entering your passwords again if an update
goes wrong. In order to do that, press the \textbf{“BACKUP”} button and a file called
\textit{prefs.ToM} will be downloaded on your device. Sometimes a pop-up opens to
warn you that this file has a strange extension, hit the \textit{Keep} button and download it anyway.\\

The opposite process is easy as well. Press the \textbf{“Browse\ldots”} button and navigate your device
folders until you find the preferences file \textit{prefs.ToM}. To apply the changes
press the \textbf{“RESTORE”} button and you will be redirected to the landing page as a confirm.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/prefs_page.png}
    \caption{OTA Update page --- Preferences backup table.}
\end{figure}

This next table is very specific to users of older ToM versions that have now received
the password to upgrade their device to ToM+ without changing the black box. If this
is your case, please type the password in the text field and press the \textbf{“UPGRADE”} button.
From now on, your ToM is finally upgraded to ToM+, enjoy!

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/hw_upgrade.png}
    \caption{OTA Update page --- Upgrade hardware table.}
\end{figure}

Press the \textbf{“HOME”} green button at the end of the table to return to the landing page.

\clearpage

\subsection{Advanced Info \& Settings page}
\label{sec:web_advanced_info}

This page is made for various advanced settings that aren't available in the ToM+ display
settings pages discussed in Section~\ref{sec:settings_page} due to space reasons.\\

The first flag, \textbf{“Disable Auto-dimming”}, can be checked if you don't like ToM+
LCD display to adapt the brightness when turning on the light beams.
Otherwise, you can adjust the \textbf{“Dimming brightness (\%)”} expressed in percentage (up to 100\%),
and set by yourself the factor of brightness reduction when the light beams are activated.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/adv_sett_page1.png}
    \caption{Advanced Info page --- Advanced settings table.}
\end{figure}

Next, we have the \textbf{Warning messages at power-off} subsection, where you can
select the reminders (\textit{light beam} and \textit{handbrake}) and info (\textit{system errors} and \textit{charge limits})
shown on ToM+ shutdown pop-up. It looks like the one shown in the image below,
letting you know, for instance, if you forgot to engage the handbrake.
The last two check boxes are for future uses, not implemented yet.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/pop-up_finale.png}
    \caption{Warning message at power-off.}
\end{figure}

Then, there is the \textbf{“Secondary font colour”} with a color picker, used to change
the secondary font colour on ToM+ display.

Another available flag is \textbf{“Invert current value sign”}. If checked, it basically shows
a signed integer in current consumption values instead of their absolute values.\\

The little subgroup below is dedicated to the \textbf{Cheatzy} compatibility with ToM+.
You can set your \textbf{“Cheating factor”} multiplied by 10: for example, 2.0 becomes 20.
You can also select if the Cheatzy is enabled when charging or during regenerative braking
using the \textbf{“Cheat also Charge/Reku”} flag. These settings only make sense if the Cheatzy 
is connected and on, otherwise they may alter your current values incorrectly.\\

Then, there is another flag: \textbf{“Increase thermal protection threshold”}, used to
increase by 15°C the overheat safety threshold for ToM+ in warmer seasons.

The next two values are additional configurations for the ToM+ calculated \textbf{Remaining KM},
based on recent consumption sampling. Here you can adjust how often the sampling
occurs (default: one per minute) in the \textbf{“Remaining KM sampling time (min)”} and
how much the last sample affects the total calculation (default: 10\%) in the
\textbf{“Remaining KM sample weight (\%)”}.\\

Lastly we have the \textbf{“Miles to Kilometers”} flag, that helps when your BigToM
was made from a mile-based dashboard, but you want km/h as its measure unit. If enabled,
ToM+ will perform this conversion for you.

Please remember to \textbf{save your changes} by pressing the \textbf{“SAVE”} button
at the end of this table. If you don't do this, your changes will be lost when you leave the page.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/adv_info.png}
    \caption{Advanced Info page --- Advanced info table.}
\end{figure}

The table above holds data about your battery, such as stored capacity, \textbf{SOH},
\textbf{battery mileage}, total consumed kWh and about charges, such as the number of
\textit{total}, \textit{full} and \textit{partial} ones and the \textbf{maximum accepted charge power}.\\

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/item_scroll.png}
    \caption{Advanced Info page --- Items scroll table.}
\end{figure}

This last table is a really peculiar feature introduced some updates ago that allows
you to have dynamically changing items in pages of your choice. It can be applied to
\textit{battery}, \textit{motor}, \textit{gyroscope}, \textit{trip} and \textit{charging}
pages and you can choose which one using the \textbf{“Page”} field.

Then, you can select which position you would like to be scrolling the data items of
your choice. To do so, use the \textbf{“Position”} field and choose from the list the
one you prefer, referring to Figure~\ref{fig:icon_arrangement} for the layout arrangement.\\

The \textbf{“Freq”} field is the scroll frequency, basically how much time each data
will stay in that position before ciclying to the next one. As you may notice, it is
specified in brackets that the field will be multiplied by \textit{5 seconds}. This
means inserting the value \textit{1} will result in a \textit{5 seconds} frequency,
the value \textit{2} will result in a \textit{10 seconds} one and so on.

The most important part is choosing the items to scroll and you can select up to
ten data values. In the \textbf{“Scrolling items”} text areas, insert the list of
desired item numbers (e.g. 59 or 48), one per each, referring to the numbers in
brackets in the list of Section~\ref{sec:item_numbers}.\\

Always remember to save your changes by pressing the \textbf{“SAVE”} button at the end of this
table. If you don't do this, your changes will be lost when you leave the page.

If you want to \textbf{disable} the scroll feature for a specific page and position,
select these fields and then input \textit{0} in the \textbf{“Freq”} text box. Right after,
press \textbf{“SAVE”} and the scroll will disable, returning to the default values.

Press the \textbf{“HOME”} green button at the end of the table to return to the landing page.



\subsection{Charging Preferences \& Log}
\label{sec:web_charging_prefs}

This page is split into two main tables: the first one is used to control the charging
process, while the second one holds a list of the last 15 charging with some additional data.

\subsubsection{Charging preferences}

Starting from the most useful ones, we have a \textbf{“Charging status”} field that holds information about
the current status of the car, including \textit{220V main} and \textit{charging process}
with their ON and OFF flags.

Just below there is the \textbf{“Charge power limit”} box, where you can choose how
much power your Twizy can use while charging, ranging from $0.4 kW$  up to $2.3 kW$ with
7 levels to choose from (select \textit{0 to disable the power limit}).

In the top right corner there is the \textbf{“ABORT CHARGING”} button, that instantly
stops gracefully the charging process, just like the one on ToM+ display in the settings
page we discusses in Section~\ref{sec:settings_page}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/charging_process.png}
    \caption{Charging Preferences page --- Charging process control table.}
\end{figure}

Right after there is the \textbf{“Stop charge at SOC \%”} field, that is the charge
percentage at which ToM+ automatically sends the signal to abort the charging process.
Just for you to know, limiting the maximum charge range (95\% or less) will slow down battery SOH decay.

Moving on to the next field, we have the \textbf{“Full charge after”} field that is
strictly related to the last one we discussed. Basically, this won't stop charge at
the selected SOC once every \textit{X} charges (\textit{for instance 10} as in the image).
Bear in mind that a 100\% charge once in a while is helpful to rebalance battery cells voltages.

Another related field is the \textbf{“Max charge once (next/current)”} flag, that works
like a button and tells ToM+ to perform a full charge, without chosen SOC stopping. This
overrides the previous field and will be applied, \textbf{after pressing “SAVE”}, to the
current charging process if it is already in progress or to the next one if not.\\


Now there is a safety field, the \textbf{“Charger max temperature”}, where you can
set a threshold (integer value expressed in °C), that if passed with an user-defined margin
hysteresis, ToM+ slowly reduces the power limit level we saw before. It keeps reducing
it, by two levels if needed, until the temperatures returns normal and so will do the power limit.

The aforesaid margin is customizable in the appropriate \textbf{“Temp margin”} field, where
you can input an integer value (expressed in °C), which will be added to the maximum
charger temperature to loose the threshold before reducing charging power.

Another related field is the \textbf{“Abort temperature”}, that holds the last safety threshold
for the charger temperature, that if passed makes ToM+ abort the charge immediately.\\


Next we have three MQTT control fields, distinguished by their progressive number.
By checking one of this flag allows you to configure remotely the parameter on its left
using the MQTT protocol. To achieve this, after enabling the appropriate field and
\textbf{pressing “SAVE”}, you will just need to publish a MQTT message on the topic
specified between the brackets (e.g. \textit{001} or \textit{002}) on your broker
(configured in Section~\ref{sec:mqtt_connection}).

If you enable the \textbf{MQTT control (001)}, you can start and stop
the charging publishing either \textit{0} or \textit{1} on the MQTT topic \textit{001}.
Same is for \textbf{MQTT control (002)} publishing on \textit{002} a value between
\textit{0} and \textit{7} and \textbf{MQTT control (003)} on \textit{003} with a value
between \textit{0} and \textit{100}.\\

Last but not least, we have the \textbf{“Aux Cmd”} field, that stands for auxilliary
command, and behaves like the one we already discussed in the alarm trigger page in
Section~\ref{sec:web_alarm_triggers}. It is sent when the charging is complete or
when the charge stops at the specified SOC.\\


After any change, press the \textbf{“SAVE”} button to \textit{save your changes}, otherwise they will be lost when you leave the page.

\subsubsection{Charging history}

The charging history table holds the last fifteen charging processes along with some
useful values to enrich them. As you can see, there is the date and time, along with
the \textbf{ODO Kilometers} value when the charging started. Next to it, we have how
much time the charging took and the \textbf{Average Power}, along with the \textbf{charged kWh}.
Finally we have the \textit{init SOC} and \textit{end SOC}, that are the starting and ending percentage.\\


The last column holds some additional \textbf{Notes} about the charging status, such
as how did it stop (either stop SOC reached or manual aborted) or if it's still going.

If you want you can manually delete some records too, pressing the \textit{X} button
at the end of the desired record. It is possible to delete all of them at once by
pressing the \textbf{“CLEAR HISTORY”} button located in the bottom part of the table.


\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/charging_history.png}
    \caption{Charging Preferences page --- Charging history table.}
\end{figure}

Press the \textbf{“HOME”} green button below the last table to return to the landing page.

\clearpage

\subsection{Time \& Date page}
\label{sec:web_time_date}

With your ToM+ you can check the current date and time, but how to set it?
This page was made for this exact purpose, and holds a single table shown in the image below.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/time_page.png}
    \caption{Current Time \& Date table.}
\end{figure}

The first five fields are self-explanatory and intuitive, just adjust date and time
as in your timezone. The \textbf{“DST”} checkbox, that stands for Daylight Saving Time,
allows you to enable or disable the 1-hour offset depending on current season.

When you want to confirm your changes and apply them to ToM+ just press the \textbf{“SET”}
button and they will be updated.\\

Next we have a very useful field, the \textbf{“Synchronize clock with WiFi”} flag,
useful when you have some WiFi connection already configured as discussed in
Section~\ref{sec:add_wifi_profile}. Just turn this on and leave ToM+ making the hard work.

Just below there is a more technical value about the date-time that tells ToM+ what's
your current time zone (\textbf{TZ}) using a string called \textbf{“POSIX”}. This includes when the DST
1-hour offset is applied. In most of center European countries the string is the following
one: \textit{CET-1CEST,M3.5.0/2,M10.5.0/3}, search online for your country POSIX TZ string if unsure.\\


As always, remember to save your changes pressing the \textbf{“SAVE”} button at the
end of the table, otherwise they will be lost when you leave the page.

Press the \textbf{“HOME”} green button below the table to return to the landing page.

\clearpage

\subsection{BigToM / ToM+ Settings page}
\label{sec:web_btom_tom_sets}

In this page you can customize some parameters of you BigToM or ToM+, because this
page mainly holds configurations for the \textbf{dashboard page}, available only in these
two devices.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/btom_page.png}
    \caption{BigToM \textbackslash ToM+ Settings table.}
\end{figure}

When replacing your dashboard with a BigToM one, you may have to add or substract
some kilometers to the new dashboard ODO to have your original one. That's because
they are stored in the dashboard motherboard. So, the first parameter is the \textbf{“ODO
kms offset”} field, used to fix this kilometers difference.

As you may know, dashboard speed indicators are incresed by $3 km/h$ on purpose for
safety reasons. If you want to see your real speed, just enable the \textbf{“Real 
speed (not ODO)”} flag in the second row of the table.\\

Now let's have a look at the \textbf{“Lock touch while driving”} flag. If active,
the touchscreen will be disabled while driving (i.e.~speed $>0$) for safety reasons.

Next, we have the \textbf{Skip welcome animation} flag, that can be enabled if you
are bored of seeing the welcome animation when changing from the dashboard to another page.\\


Then, there is a section dedicated to the power bar shown in Figure~\ref{fig:dashboard_layout}
at the top of the dashboard layout. Here you can choose, using the \textbf{“Power bar item”}
choice box, what value should be shown and used to fill the power bar. There are three
available data you can use: \textit{Engine Torque}, \textit{Battery Ampere} or \textit{Battery kW}.

The two \textbf{“Up/Down section scale \%”} fields below are used to adjust the range of the
power bar and represent the percentage of the maximum/minimum value of the selected data item.
In the example image, for the maximum range I put \textit{30\%}.\\


In the next section we can enable another cool feature, shown in Figure~\ref{fig:dashboard_layout}
as well, right below the speed indicator. This is achieved activating the \textbf{“Enable
alert speed bar”} flag, that has three customizable thresholds, each one corresponding
to a color from \textit{green} to \textit{yellow} and \textit{red}.

The following three fields, \textbf{“Speed alert low/medium/high limit”}, are meant
to be filled with the respective threshold speed in $km/h$ to light up the speed bar
with the appropriate colour. In the example, the bar will become yellow when passing
$50 km/h$ and then red when passing $70 km/h$. As you may notice, I didn't want the
green bar to show up, so I put the \textit{low} threshold value the same as the 
\textit{medium} one.\\


Last but one, we have the \textbf{“3D layout for charging page”} flag, enabled by
default, that shows 3D details instead of the standard ToM charging page.

And finally there is the most recently added feature, called the \textbf{Twin ToM}
and designed for the original ToM owners, before ToM+ came out. This allows you to
use the smaller display along with the new clip-on version, in order to monitor
two pages simultaneously. But to make this work you need to enable the last row
flag, the \textbf{“Enable auxiliary display”} one.

Then you can select, with the \textbf{“Start page”} choice box, a specific page to
show on the auxillary display at power on, or simply choose \textit{Last shown}.\\
...
\subsection{Diagnostic page}
\label{sec:web_diagnostic}

This page is composed by one table only and allows you to view more information
about the last 10 errors that occured in your Twizy, similar to the ToM+ display
page discussed in Section~\ref{sec:error_page}.

The shown values are the \textbf{“DTC Number”} and, if available, the \textbf{“DF Number”}
that are unique identifiers for Twizy errors, so that you can search online for it.
Next we have the error \textbf{“Status”}, such as \textit{SAVED} if already happened
or \textit{PRESENT}, if it has just occurred.

To \textit{enable DF Numbers}, please refer to Section~\ref{sec:add_wifi_profile}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{figures/error_page.png}
    \caption{Diagnostic page --- Error list table.}
\end{figure}

Along with these values, the next four values are the reference \textbf{“ODO”},
\textbf{“SOC”}, \textbf{“speed”} and \textbf{“12V Battery voltage”} when the
corresponding error occured.

For each error record, if available, there is an error \textbf{“Description”} field.\\


If you would like to clear Twizy errors, even fatal ones, you can press the below
\textbf{“DELETE ERRORS”} button, that will delete all of them at once.
