...
{Min-Max battery cell difference (\(V\))}
...
{Charger temperature (\textdegree C)}
...
{Trip kilometers left (\(km\))}
{The remaining kilometer range with the current trip consumption.}

\iconbox{icons/tkWh.png}
{Trip total power consumption (\(kWh\))}
{The total power consumption in the current trip.}

\iconbox{icons/tkWh1.png}
{Trip power consumption per kilometer (\(kWh/km\))}
{The current power consumption per kilometer in the current trip.}

\iconbox{icons/tkWh100.png}
{Trip power consumption per 100 kilometers (\(kWh/100km\))}
{The current power consumption per 100 kilometers in the current trip.}

\iconbox{icons/tkWh1min.png}
{Trip power consumption per minute (\(kWh/min\))}
...
{Trip uphill average inclination (\textdegree)}
{The average uphill inclination in the current trip.}

\iconbox{icons/tDownhill.png}
{Trip downhill average inclination (\textdegree)}
...
\item[\textbf{11}] Displays the time and date, cycle between them by tapping on it. \todo{BAR}
\end{description}
...
to change the foreground data and more. It will be described in detail in Section~\ref{sec:wiper_stalk}.
...
below the 30\%. You will discover how to set your own alarm on the web server in Section~\ref{sec:alarm_triggers}.
...
the alarm log icon} in the bottom line of the screen until it changes. This is because the trip history page
...
each one with the respective icon.\\
...
On the web server you can see all of them at once, it will be discussed in Section~\ref{sec:trip_log_web}.
...
This page is not accessible from the main page, but you can reach it by \textbf{long pressing 
the alarm log icon} in the bottom line of the screen until it changes. As we discussed before, the sequence of pages available by long pressing the alarm log icon are:
alarm log page (Section~\ref{sec:alarm_log}) $\rightarrow$ error page $\rightarrow$ trip history page (Section~\ref{sec:trip_history}) $\rightarrow$ back to alarm log page,
each one with the respective icon.\\
...
the web server for more information such as SOC, speed and a brief description (Section~\ref{sec:error_page_web}).\\
...
\end{itemize}
You can update to the latest version of the firmware as explained in Section~\ref{sec:firmware_updates}.
