...
\textit{Dies ist das offizielle ToM+-Benutzerhandbuch, das von mir (Fabio Java) eigenhändig in englischer Sprache verfasst wurde, ohne den Einsatz
von KI-Tools – weder in diesem Handbuch noch im Code, auf dem ToM+ basiert. Andere Sprachen
als Englisch wurden automatisch mithilfe von} \href{https://www.deepl.com/en/translator}{DeepL} übersetzt.\\
...
bei Bedarf anderen ToM+-Besitzern Support zu garantieren.}\\

\textit{ToM+ entwickelt sich dank seiner großen Community, die aus dem} \href{https://www.twizy-forum.de/twiz-o-meter}{deutschen Twizy-Forum} hervorgegangen ist, ständig weiter. \textit{Wenn Sie 
...
Da das Armaturenbrett unseres Twizy nur einige wenige Werte anzeigt, wie beispielsweise die Geschwindigkeit und die
...
Ihre Konfigurationen und individuellen Einstellungen auf dem ToM zu speichern – auch nach einem Firmware-Update.
...
Wie beim Original werden der Akkustand, der Gang, die verbleibende Reichweite in Kilometern, die Geschwindigkeit und die Uhrzeit angezeigt.
Ich habe jedoch den genauen \textbf{Akkustand in Prozent}, das Motordrehmoment, einige Statussymbole, Fahrteninformationen und viele weitere Elemente hinzugefügt, die Sie nach Belieben konfigurieren können und auf die wir später noch im Detail eingehen werden.
...
aktiv zu lassen, auch wenn keine Internetverbindung besteht (dies wird in der Regel bei
Android-Smartphones 10 Sekunden nach der ersten Verbindung durch eine Benachrichtigung angezeigt), um anschließend Ihre
weiteren Konfigurationen auf einigen Webseiten vorzunehmen, die von ToM+ selbst gehostet werden. Geben Sie einfach
...
und ignorieren Sie die Meldung (die App funktioniert einwandfrei). Hier eine Vorschau auf die Lade- und Einstellungsseite der App:
...
verbraucht dabei nur wenig Strom, wie die folgenden Messwerte zeigen.
...
Wenn die Hauptplatine aktiv ist und das Display eingeschaltet ist, beträgt der Stromverbrauch etwa 90 mA. Ist zudem das WLAN eingeschaltet, liegt der Stromverbrauch in etwa bei dem unten angegebenen Wert (möglicherweise etwas höher aufgrund der letzten Updates, mit denen die 3D-Grafik eingeführt wurde).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figures/consumption_on_wifi.png}
    \caption{Stromverbrauch, wenn ToM+ eingeschaltet ist und WLAN aktiv ist}
\end{figure}
...
Finger oder einen kleinen Stylus aus hartem Kunststoff verwenden, um die Tasten zu betätigen. Es ist zwar nicht so leistungsstark wie
...
Es werden keine Schrauben oder Klebeband mehr benötigt! Nur noch ein einfaches Clip-On-Display, das direkt auf das ursprüngliche Armaturenbrett passt.
...
hier abgebildeten Stecker tun. Tatsächlich stellt er eine zusätzliche 5-V-Stromversorgung bereit, die von
...
Das resistive Touch-LCD-Display ist in einen 3D-gedruckten Ständer eingebettet, der auf das Original-Armaturenbrett aufgesteckt wird. Auf Wunsch erhalten Sie auch eine eigenständige 3D-Halterung, mit der Sie das Display wie das Standard-ToM am Handschuhfach befestigen können. Auf diese Weise verfügen Sie über ein 2-in-1-Gerät, das je nach Wunsch sowohl aufgesteckt als auch eigenständig verwendet werden kann. Für Firmware-Updates steht ein microSD-Steckplatz zur Verfügung.
...
Das Display, das – wie auf den Abbildungen zu sehen – mit einer Schutzfolie versehen ist, kann in den 3D-Behälter eingesetzt werden, um einen verstellbaren Ständer zu erhalten, den Sie an einer beliebigen Stelle aufstellen können.
...
In der Standalone-Version können Sie das Display mit der Abdeckung und den Schrauben befestigen, die Sie zusammen mit dem zusammenklappbaren 3D-Ständer in der Verpackung vorfinden, wie in den folgenden Abbildungen dargestellt.
...
Der 3D-Ständer verfügt außerdem über Löcher an der Basis, falls Sie ihn am Handschuhfach befestigen möchten. Die übrigen Löcher am Rahmen dienen zur Befestigung der Rückabdeckung, um das Display im Inneren an seinem Platz zu halten.
